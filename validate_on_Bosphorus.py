...
def evaluate_bd415(distances, labels, num_folds=10, far_target=1e-1):
    """Evaluates on the Bosphorus + D415 dataset using KFold cross validation based on the Euclidean
    ...
    thresholds_roc = np.arange(0, 6, 0.01)
    ...
    thresholds_val = np.arange(0, 6, 0.001)
...
def calculate_val(thresholds_val, distances, labels, far_target=1e-1, num_folds=10):
    num_pairs = min(len(labels), len(distances))
